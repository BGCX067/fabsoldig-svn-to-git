...
public class CampaingAction extends DispatchAction{
	...
	public ActionForward campaingInfo(ActionMapping mapping, ActionForm form,
		...
		try {
		    ...
		    //isHuman = captcha.validate(request, request.getParameter("txtCaptcha"));
		    isHuman = captcha.validate(request, request.getParameter("txtCaptcha"));
		    System.out.println(request.getParameter("txtCaptcha"));
		    logger.info("captcha: " + request.getParameter("txtCaptcha"));
		    ...
		    if(isHuman) {
				...
				datosCliente.put("autorizacion", campaingForm.getChkAutTDP());
				...
				try {
						...
						*/
						if("999".equalsIgnoreCase(clienteBase.get("TIP_CLIENTE"))) {
				...
				try {
					parametrosService.enviarSMS(datosCliente);
				} catch (Exception e) {
					logger.error(GenericObject.formatException(e));
				}
				*/
		    	...
		    	redireccion = "campaingForm";
		    	request.setAttribute("campaingForm", form);
		    }
	...
	public String saveCampaingAjax(Map<String, String> datosCliente) {
		...
		try {
			...
			logger.info("clienteInscrito: " + clienteInscrito);
		} catch(Exception e) {
			logger.error(GenericObject.formatException(e));
		}
	...
	public String validateCaptchaAjax(String txtCaptcha, HttpServletRequest request) {
		boolean isResponseCorrect = Boolean.FALSE;
		try {
            isResponseCorrect = SimpleImageCaptchaServlet.validateResponse(request, txtCaptcha);
        } catch (CaptchaServiceException e) {
        	logger.error(GenericObject.formatException(e));
        }
		
		logger.info("RESPUESTA DEL CAPTCHA ("+txtCaptcha+"): " + isResponseCorrect);
		
		return isResponseCorrect?"OK":"ERROR";
	}
	
	
	public String validateCaptcha2(String txtCaptcha, HttpServletRequest request) {
			boolean isHuman = false;
		    Captcha captcha = Captcha.load(request, "exampleCaptchaTag");
		    //isHuman = captcha.validate(request, request.getParameter("txtCaptcha"));
		    isHuman = captcha.validate(request, txtCaptcha);
		    System.out.println(txtCaptcha);
		    System.out.println(request.getParameter("txtCaptcha"));
		    return isHuman?"OK":"ERROR";
	}
	
	/*public ActionForward validateCaptcha2(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) throws Exception {
		 boolean isHuman = false;
		if(request.getParameter("validate") != null){
		      ...
		      isHuman = captcha.validate(request, request.getParameter("captchaCode"));
